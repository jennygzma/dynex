...
def get_ui_code(plan, task, spec, previous_task_main_code_file_path, task_merged_code_file_path, faked_data_info, code_rules):
...
"""
	...
	if previous_code_lines-10 > merged_code_lines:
		...
		get_ui_code(plan, task, spec, previous_task_main_code_file_path, task_merged_code_file_path, faked_data_info, code_rules)
...
def implement_plan_lock_step(spec, plan, code_folder_path, task_id, faked_data, tools_requirements):
	...
	# below is for Claude
	get_ui_code(plan, step["task"], spec, previous_task_main_code_file_path, task_merged_code_file_path, faked_data_info, code_rules)
	cleanup_code(task_cleaned_code_file_path, task_merged_code_file_path, task_main_code_file_path)
	print("finished executing lock step for task_id", {task_id})

def implement_first_task(spec, task, task_merged_code_file_path, faked_data_info, code_rules):
	print("calling LLM for implement_first_task...")
	user_message = f"Please execute this task: {task}."
	system_message = f"""
    You are writing HTML, Javascript, and CSS code for creating a UI given a data model. For context, this is the goal: {spec}.
		{code_rules}
    {faked_data_info}
"""
	code = call_llm(system_message, user_message)
